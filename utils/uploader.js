...
const uploadServerURL='http://localhost:7777/upload'
const uploader=async(files,id)=>{   
// ==============================================
...
try{
    setTimeout(async() => {        
        let data=await Promise.all(files.map(async(file)=>{    
          ...
          data.append('sampleFile',fs.createReadStream(path.join(__dirname, '../','/documents/load',file.filename)));          
          var config = {
            method: 'post',
            url:uploadServerURL,
            headers: { 
